...
bool solve() {
  ...
  vector<int> choice(5, 0);
  REP(i, 2) choice[5 - 2 + i] = 1;  // Fill last k items.
  do {
    vector<int> d;
    REP(i, 5) if (choice[i]) d.push_back(i);
    assert(ssize(d) == 2);
    array<int, 4> cnt = {0, 0, 0, 0};
    REP(i, n) {
      bool ok0 = a[i][d[0]];
      bool ok1 = a[i][d[1]];
      if (ok0 and ok1) cnt[3]++;
      else if (ok0) cnt[1]++;
      else if (ok1) cnt[2]++;
      else cnt[0]++;
    }
    if (cnt[0] == 0 and cnt[1] <= n / 2 and cnt[2] <= n / 2) {
      return true;
    }
