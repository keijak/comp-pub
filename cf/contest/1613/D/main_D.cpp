...
auto solve() {
  ...
  auto dp = vector(n, vector(3, Mint(0)));
  auto dps = vector(n + 5, vector(3, Mint(0)));
  for (int i = n - 1; i >= 0; --i) {
    Mint nojump = 1;
    nojump += dps[a[i]][0];
    nojump += dps[a[i] + 1][0];
    ...
    int z1 = ssize(indices[a[i]]);
    int z2 = (a[i] >= 2) ? ssize(indices[a[i] - 2]) : 0;
    Mint vibrate = Mint(2).pow(z1 + z2);
  ...
  Mint ans = 0;
  for (auto j: indices[0]) {
    ans += dp[j][0] + dp[j][1];
  }
  ans += Mint(2).pow(ssize(indices[1])) - 1;
  return ans;
}
