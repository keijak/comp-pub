...
auto solve() {
  ...
  vector<int> t = in(n);
  sort(ALL(t));
  atcoder::mcf_graph<int, int> g(n * 3 + 2);
  const int kSource = n * 3;
  const int kSink = kSource + 1;
  REP(v, n) {
    g.add_edge(kSource, v, 1, 0);
  }
  REP(v, n, n * 3) {
    g.add_edge(v, kSink, 1, 0);
  }
  REP(v, n) {
    int target = t[v];
    REP(actual, 1, n * 2 + 1) {
      int u = n + (actual - 1);
      int cost = abs(target - actual);
      g.add_edge(v, u, 1, cost);
    }
  }
  auto [flow, cost] = g.flow(kSource, kSink);
  DUMP(flow, cost);
  print(cost);
}
