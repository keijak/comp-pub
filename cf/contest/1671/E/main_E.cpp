...
std::string bin(long long x) {
  if (x == 0) { return "0"; }
  std::string res;
  for (; x; x >>= 1) { res.push_back((x & 1) ? '1' : '0'); }
...
auto solve() {
  int n = in;
  string S = in;
  assert(ssize(S) == (1 << n) - 1);
  auto [ans, minset] = Rec([&](auto f, int x, int d) -> pair<Mint, string> {
    if (d == n - 1) {
      return {1, string(1, S[x - 1])};
    }
    auto [c1, s1] = f(x * 2, d + 1);
    auto [c2, s2] = f(x * 2 + 1, d + 1);
    string s(1, S[x - 1]);
    if (s1 <= s2) {
      s += s1;
      s += s2;
