...
auto solve() {
  int K = in, A = in;
  unsigned H = in;
  vector<int> P = {1, 2, 3, 5, 9, 17};
  vector<int> pc = {1, 1, 2, 4, 8, 16};
  vector<int> ans(1 << K);
  vector<Mint> hval(1 << K);
  set<int> pending, determined;
  REP(i, 1 << K) pending.insert(i);

  auto powa = pow_seq<Mint>(20, A);
  const unsigned kFull = (1 << (K + 1)) - 1;

  auto f = [&](auto &f, int i, int n) -> void {
    if (i == K + 1) {
      Mint hsum = 0;
      REP(j, 1 << K) {
        hsum += hval[j];
    ...
    do {
      ...
      unsigned mask = ~(kFull >> (i + 1));
      set<unsigned> dmasked;
      for (auto x: determined) {
        dmasked.insert(x & mask);
      }
      if (ssize(dmasked) == ssize(determined)) {
        f(f, i + 1, n - c);
      }
      for (auto q: chosen) {
        ans[q] = 0;
