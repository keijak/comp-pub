...
auto solve() {
  ...
  vector<int> sizes(n);
  Rec([&](auto &rec, int v, int p, int d) -> int {
    int ret = 1;
    for (auto u: g[v]) {
      if (u == p) continue;
      ret += rec(u, v, d + 1);
  ...
  int ans = Rec([&](auto &self, int v, int p) -> int {
    int num_children = ssize(g[v]) - (p == -1 ? 0 : 1);
    if (num_children == 0) return 0;
    if (num_children == 1) return sizes[v] - 2;
    int ret = 0;
    int usum = accumulate(ALL(g[v]), 0);
    for (auto u: g[v]) {
      if (u == p) continue;
      int other = usum - u - (p != -1 ? p : 0);
      chmax(ret, self(u, v) + (sizes[other] - 1));
    }
    return ret;
  })(0, -1);
