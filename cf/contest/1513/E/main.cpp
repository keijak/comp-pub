...
Mint solve() {
  ...
  const int nl = ssize(lo), nh = ssize(hi);
  if (nl == 0 and nh == 0) {
    return 1;
  }
  if (nl == 1 and nh == 1) {
  ...
  if (nl == 1) {
    Mint ans = n * fs.C(n - 1, nh) * fs.fact[nh];
    for (auto [x, k] : fh) ans /= fs.fact[k];
    return ans;
  }
  if (nh == 1) {
    Mint ans = n * fs.C(n - 1, nl) * fs.fact[nl];
    for (auto [x, k] : fl) ans /= fs.fact[k];
    return ans;
  }
  if (nl > 0 and nh > 0) {
    Mint ans = 0;
    for (int k = nl; k + nh <= n; ++k) {
      ans += fs.C(k - 1, nl - 1) * fs.C(n - k, nh);
    }
    Mint mul = fs.fact[nl] * fs.fact[nh];
    for (auto [x, k] : fl) mul /= fs.fact[k];
    for (auto [x, k] : fh) mul /= fs.fact[k];
    ans *= mul;
    ans *= 2;
    return ans;
  }
