...
optional<Mint> solve() {
  const Int b = in, q = in, y = in;
  const Int c = in, r = in, z = in;
  if (r % q != 0) return Mint(0);
  if (c < b) return Mint(0);
  if ((c - b) % q != 0) return Mint(0);
  const Int b_last = b + q * (y - 1);
  const Int c_last = c + r * (z - 1);
  if (c_last > b_last) return Mint(0);
  if (c_last + r > b_last) return nullopt;
  if (c - r < b) return nullopt;

  Mint ans = 0;
  for (Int p: divisors(r)) {
    Int g = gcd(p, q);
    if (p / g * __int128(q) != r) continue;
    Int k = r / p;
    ans += Mint(k) * k;
  }
