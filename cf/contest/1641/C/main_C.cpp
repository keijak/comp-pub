...
auto solve() {
  int n = in, Q = in;
  set<int> maybe_sick;
  REP(i, n) maybe_sick.insert(i);
  maybe_sick.insert(-1000);
  maybe_sick.insert(n + 1000);

  SegmentTree<MinOp> seg(n);
  REP(qi, Q) {
    int t = in;
    if (t == 0) {
      int l = in, r = in, x = in;
      --l;
      if (x == 0) {
        for (auto it = maybe_sick.lower_bound(l); it != maybe_sick.end() and *it < r;) {
          it = maybe_sick.erase(it);
        }
      } else {
        seg.set(l, min(r, seg[l]));
      }
    } else {
      assert(t == 1);
      int j = in;
      ...
      if (not maybe_sick.count(j)) {
        print("NO");
        continue;
      }

      auto itr = maybe_sick.upper_bound(j);
      auto itl = maybe_sick.lower_bound(j);
      assert(*itl == j);
      int l = *--itl;
      int r = *itr;
      int z = seg.fold(max(l + 1, 0), j + 1);
      if (z <= r) {
        print("YES");
      } else {
        print("N/A");
