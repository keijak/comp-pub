...
Int solve() {
  ...
  Int left_most = bisect(0LL, -kInf, [&](Int x) -> bool {
    Int pos = 0;
    bool reached = false;
    REP(i, n) {
      if (a[i] == 0) {
    ...
    return reached and pos >= 0;
  });
  Int right_most = bisect(0LL, kInf, [&](Int x) -> bool {
    Int pos = 0;
    bool reached = false;
    REP(i, n) {
      if (a[i] == 0) {
