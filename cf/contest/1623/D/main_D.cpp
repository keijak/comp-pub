...
auto solve() {
  int n = in, m = in, rb = in, cb = in, rd = in, cd = in, P = in;
  --rb, --cb, --rd, --cd;
  set<int> rx, cx;
  rx.insert((rd - rb + 2 * (n - 1)) % (2 * (n - 1)));
  if (rd != 0 and rd != n - 1) {
    rx.insert((2 * (n - 1) - rb - rd) % (2 * (n - 1)));
  }
  cx.insert((cd - cb + 2 * (m - 1)) % (2 * (m - 1)));
  if (cd != 0 and cd != m - 1) {
    cx.insert((2 * (m - 1) - cb - cd) % (2 * (m - 1)));
  }
  DUMP(rx, cx);
  Int g = std::gcd((Int) n - 1, (Int) m - 1);
  Int period = lcm(2 * (n - 1), 2 * (m - 1));
  vector<Int> lx;
  for (int x: rx) {
    x %= period;
    Int offset = 0;
    REP(i, (m - 1) / g) {
      lx.push_back(offset + x);
      offset += 2 * (n - 1);
    }
  }
  for (int x: cx) {
  ...
  sort_unique(lx);
  DUMP(n, m, period, lx.size());
  if (lx.size() < 10) DUMP(lx);
  assert(not lx.empty());
  Mint q = Mint(100 - P) / 100;

  auto f = [&](auto &f, int i) -> pair<Mint, Mint> {
    if (i == ssize(lx) - 1) {
      Int x = (lx[0] - lx[i] - 1 + period) % period;
      return {Mint(1 + q * x), Mint(q)};
    } else {
      auto[x, y] = f(f, i + 1);
      x += (lx[i + 1] - lx[i] - 1);
      x *= q;
