...
bool solve() {
  ...
  int i = 0, j = n / 2;
  P v1 = points[i + 1] - points[i];
  P u1 = points[j + 1] - points[j];
  if (v1.norm() != u1.norm()) return false;
  i = (i + 1) % n;
  j = (j + 1) % n;
  while (true) {
    P v2 = points[(i + 1) % n] - points[i];
    P u2 = points[(j + 1) % n] - points[j];
    if (v2.norm() != u2.norm()) {
      return false;
    }
    if (((v2 - v1) + (u2 - u1)).norm() > 0) {
      return false;
    }
    swap(v1, v2);
    swap(u1, u2);
