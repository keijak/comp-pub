...
int main() {
  ...
  V<int> init(n + 1, 1);
  init[0] = init[n] = 0;
  SegTree<Sum> seg(init);
  assert(seg.fold(0, n + 1) == n - 1);

  auto solve = [&](int r, int c) -> bool {
    if (a[r][c]) {
      // => off
      a[r][c] = false;
      if (not a[r ^ 1][c] or not a[r ^ 1][c + 1]) {
        seg.set(c, 0);
      }
      if (not a[r ^ 1][c - 1] or not a[r ^ 1][c]) {
        seg.set(c - 1, 0);
      }
    } else {
      // => on
