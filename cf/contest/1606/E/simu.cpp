...
//  }
  for (int n = 1; n <= 7; ++n) {
    ...
    REP(p, 5) {
      REP(i, 1, ans.size()) {
        delt.push_back(ans[i] - ans[i - 1]);
      }
      ans = delt;
    }
    cerr << n << ": " << ans << endl << delt << endl;
  }
}
