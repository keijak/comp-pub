...
auto solve() {
  ...
  DUMP(ds);
  DUMP(acc);
  SegTree<MaxOp> maxseg(acc);
  SegTree<MinOp> minseg(acc);
  ...
  REP(i, q) {
    ...
    if (r - l == 1) {
      ...
      assert(x % 2 == 1);
      i64 h = acc[l] + (x / 2);
      i64 j = max_right(maxseg, l + 1, [&](i64 val) {
        return val <= h;
      });
      assert(acc[j] == h + 1);
      assert(acc[j - 1] == h);
      j -= 1;
      DUMP(l, j, r, h);
      DUMP(acc[j] - acc[l], acc[r] - acc[j + 1]);
      assert(j >= l);
      assert(j < r);
//      assert(ds[j] == 1);
//      assert(acc[j] - acc[l] == x / 2);
//      assert(acc[j + 1] - acc[j] == 1);
//      assert(acc[r] - acc[j + 1] == x / 2);
      ans.push_back(j + 1);
    } else if (x < 0) {
      assert((-x) % 2 == 1);
      i64 h = acc[l] - ((-x) / 2);
      i64 j = max_right(minseg, l + 1, [&](i64 val) {
        return val >= h;
      });
      assert(acc[j] == h - 1);
      assert(acc[j - 1] == h);
      ...
      assert(j < r);
//      assert(ds[j] == -1);
//      assert(acc[l] - acc[j] == abs(x) / 2);
//      assert(acc[j] - acc[j + 1] == -1);
//      assert(acc[j + 1] - acc[r] == abs(x) / 2);
      ans.push_back(j + 1);
    }
    print(ssize(ans));
    if (not ans.empty()) print_seq(ans);
  }
}
