...
auto solve() {
  ...
  cin >> a;
  Compressed<i64> ca(a);
  SegTree<MaxOp> seg(ca.size());
  SegTree<MaxOp> seg2l(ca.size());
  SegTree<MaxOp> seg2h(ca.size());
  ...
  REP(i, n) {
    ...
    chmax(gl, seg2l.fold(0, xi + 1));
    chmax(gl, seg2h.fold(xi, ca.size()));

    ans += (i + 1) - (gl + 1);

    int y1 = seg.fold(0, xi + 1);
    seg2l.set(xi, y1);

    int y2 = seg.fold(xi, ca.size());
    seg2h.set(xi, y2);
