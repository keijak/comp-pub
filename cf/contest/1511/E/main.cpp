...
vector<i64> components(const vector<char> &a) {
  vector<i64> res;
  const int n = a.size();
  int i = 0;
  while (true) {
...
Mint solve() {
  ...
  vector<Mint> pow2(n * m + 1);
  pow2[0] = 1;
  REP(i, n * m) { pow2[i + 1] = pow2[i] * 2; }

  const int L = max(n, m);
  vector dp(2, vector(L + 1, Mint(0)));
  dp[1][1] = 1;
  for (int k = 2; k <= L; ++k) {
    dp[1][k] = pow2[k - 1] + 2 * dp[0][k - 1];
    dp[0][k] = pow2[k - 2] + 3 * dp[0][k - 2] + dp[1][k - 2];
  }

  Mint ans = 0;
  REP(rot, 2) {
    for (const auto &row : g) {
      for (auto q : components(row)) {
        ans += dp[0][q] * pow2[w - q];
      }
    }
    rotate90(g);
