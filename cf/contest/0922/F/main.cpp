...
std::vector<bool> prime_sieve(int n) {
  ...
  const int nr = std::lround(std::ceil(std::sqrt((double)n)));
...
optional<set<int>> solve() {
  i64 n, k;
  cin >> n >> k;

  auto upper = [&](i64 z) -> i64 {
    i64 tot = 0;
    for (i64 x = 1; x < z; ++x) {
      i64 d = z / x;
      if (d <= 1) break;
  ...
  int m = tv;
  auto is_prime = prime_sieve(m);
  i64 delta = tot - k;
  assert(delta >= 0);
  set<int> ans;
  for (int x = 1; x <= m; ++x) ans.insert(x);
  for (int x = 1; x <= m; ++x) {
    if (delta == 0) break;
