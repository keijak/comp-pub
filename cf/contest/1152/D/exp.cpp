...
pair<i64, bool> dfs(int a, int b, bool close) {
  if (a == n and b == n) {
    return {0LL, false};
  }
  if (a == b) {
    auto [p, q] = dfs(a + 1, b, false);
    if (not q) return {p + 1, true};
    return {p, false};
  } else {
    assert(a > b);
    i64 p1 = 0, p2 = 0;
    ...
    if (a < n) {
      tie(p1, q1) = dfs(a + 1, b, false);
    }
    if (b < n) {
      tie(p2, q2) = dfs(a, b + 1, true);
    }
    if (not q2 and not q2) {
      return {p1 + p2 + 1, true};
    }
    return {p1 + p2, false};
  }
...
template <typename T>
T catalan(int k) {
  auto ret = comb<Int>(2 * k, k);
  if (k > 0) ret -= comb<Int>(2 * k, k - 1);
  return ret;
}

Int solve() {
  Int res = dfs(1, 0, false).first;
  for (int i = 1; i <= n; ++i) {
    res -= catalan<Int>(i);
  }
  return res;
