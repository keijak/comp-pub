...
vector<vector<optional<pair<Mint, int>>>> dp;

pair<Mint, bool> dfs(int a, int b) {
  if (a == n and b == n) {
    return {0, false};
  }
  if (dp[a][b].has_value()) {
  ...
  if (a == b) {
    ...
    if (not q2 and not q2) {
      dp[a][b] = pair{p1 + p2 + 1, true};
    } else {
      dp[a][b] = pair{p1 + p2, false};
    }
  }
  return dp[a][b].value();
}
...
Mint solve() {
  cin >> n;
  dp.assign(n + 1, vector<optional<pair<Mint, int>>>(n + 1));
