...
Int solve() {
  ...
  assert(ssize(s) == n);
  const int L = n * 2 + 10;
  const int kOffset = n + 5;
  vector<SegmentTree<SumOp>> st;
  REP(i, 3) {
    st.emplace_back(L);
  }
  st[0].set(kOffset, 1);
  ...
  REP(i, n) {
    ...
    Int delta = st[(c + B) % 3].fold(kOffset + c + 1, L);
    ans += delta;
    auto val = st[(c + B) % 3][kOffset + c];
    st[(c + B) % 3].set(kOffset + c, val + 1);
//    DUMP(i, s[i], c, delta, ans);
//    DUMP(st[0].to_vec());
//    DUMP(st[1].to_vec());
//    DUMP(st[2].to_vec());
  }
