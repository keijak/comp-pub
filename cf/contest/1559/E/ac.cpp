...
using DF = DenseFPS<NTTMult<Mint, DMAX>>;
...
FPS &operator+=(FPS &x, const SparseFPS<typename FPS::T> &y) {
  for (int i = 0; i < y.size(); ++i) {
    if (y.degree(i) > FPS::RMAX) break;  // ignore
...
FPS &operator*=(FPS &x, const SparseFPS<typename FPS::T> &y) {
  ...
  const int n = std::min(xd_max + yd_max + 1, RMAX + 1);
...
std::vector<int> primes_upto(int n) {
  std::vector<int> res;
  spf = sieve_smallest_prime_factors(n);
  for (int i = 2; i <= n; ++i) {
