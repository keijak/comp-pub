...
auto solve() {
  ...
  vs.push_back(Max::id());
  Compressed<i64> cvs(vs);

  SegTree<Max> seg(cvs.size());
  seg.set(0, 0);

  for (const auto &[u, vs] : uv) {
    map<i64, i64> upd;
    for (auto v : vs) {
      int i = cvs.index(v);
      i64 maxlevel = seg.fold(0, i);
      if (auto it = upd.find(i); it != upd.end()) {
        chmax(it->second, maxlevel + 1);
      } else {
        upd.emplace(i, maxlevel + 1);
      }
    }
    for (auto [i, lv] : upd) {
      seg.set(i, max(seg[i], lv));
    }
  }
  return seg.fold_all();
}
