...
i64 solve() {
  ...
  unordered_map<i64, i64> mincost;
  mincost.reserve(1 << 20);
  mincost.max_load_factor(0.25);
  mincost[Y] = 0;
  ...
  while (not que.empty()) {
    ...
    que.pop();
    if (cur.cost > mincost[cur.val]) continue;
    if (cur.val == X) {
      return cur.cost;
    }
    ...
    for (auto newval : newvals) {
      ...
      if (it == mincost.end() or it->second > newcost) {
        mincost[newval] = newcost;
        que.push({newcost, newval});
      }
    }
  }
