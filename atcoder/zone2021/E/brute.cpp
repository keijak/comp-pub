...
i64 solve() {
  ...
  while (not que.empty()) {
    ...
    if (cur.r < R - 1) {
      ...
      push(new_cost, cur.r + 1, cur.c);
    }
    for (int i = 1; cur.r - i >= 0; ++i) {
      i64 new_cost = cur.cost + (1 + i);
      push(new_cost, cur.r - i, cur.c);
    }
  }
