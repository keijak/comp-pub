...
const unsigned int MOD = 998'244'353;
...
V<Mint> powmod(200002);
V<Mint> cum(200002);

struct ReplaceDigits {
  struct T {
    Mint val;
    int lsb;
    int msb;
  ...
  static T op(const T &x, const T &y) {
    int msb = max(x.msb, y.msb);
    int lsb = min(x.lsb, y.lsb);
    return {x.val + y.val, lsb, msb};
  }
  static constexpr T id() { return {0, (int)1e8, -1}; }

  // Update: Assign
  static T f_apply(const F &f, const T &x) {
    if (not f.has_value()) return x;
    Mint val = (cum[x.msb + 1] - cum[x.lsb]) * Mint(f.value());
    return {val, x.lsb, x.msb};
  }
...
int main() {
  ...
  V<ReplaceDigits::T> A(N);

  REP(i, N) { A[N - 1 - i] = {powmod[i], i, i}; }
  LazySegTree<ReplaceDigits> st(A);
  REP(i, Q) {
    int l, r, d;
    cin >> l >> r >> d;
    l--;
    st.apply(l, r, d);
    cout << st.fold(0, N).val << '\n';
  }
}
