...
auto solve() {
  ...
  segs.emplace_back(m + 1);
  vector<int> rowt(n, 0), rowx(n, 0);
  for (int t = 1; t <= Q; ++t) {
    int qt = in;
    if (qt == 1) {
      int l = in, r = in, x = in;
      --l;
      auto &s0 = segs.back();
      auto s1 = s0.set(l, s0[l] + x);
      auto s2 = s1.set(r, s1[r] - x);
      segs.push_back(s2);
    } else if (qt == 2) {
      int i = in, x = in;
      --i;
      ...
      int rt = rowt[i];
      Int rx = rowx[i];
      Int d1 = segs.back().fold(0, j + 1);
      Int d2 = segs[rt].fold(0, j + 1);
      //DUMP(t, i, j, rt, rx, d1, d2);
      print(rx + d1 - d2);
      segs.push_back(segs.back());
    }
  }
