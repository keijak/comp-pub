...
auto solve() {
  int n = in, X = in, Y = in;
  vector<Int> a = in(n);
  SegmentTree<MaxOp> seg_max(a);
  SegmentTree<MinOp> seg_min(a);

  Int ans = 0;
  REP(l, n) {
    int r1_begin = max_right(seg_max, l, [&](const auto &x) {
      return x < X;
    });
    int r1_end = max_right(seg_max, l, [&](const auto &x) {
      return x <= X;
    });
    int r2_begin = max_right(seg_min, l, [&](const auto &x) {
      return x > Y;
    });
    int r2_end = max_right(seg_min, l, [&](const auto &x) {
      return x >= Y;
    });
    int L = max(r1_begin, r2_begin);
    int R = min(r1_end, r2_end);
    if (L <= R) ans += R - L;
  }
  print(ans);
}
