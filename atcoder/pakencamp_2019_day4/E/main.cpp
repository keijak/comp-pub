...
Mint solve() {
  ...
  REP(i, m) {
    ...
    cin >> a >> b >> s >> w;
    --a, --b;
    g[a].push_back(Edge{s, w, b});
    g[b].push_back(Edge{s, w, a});
  }

  vector mincost(3, vector<i64>(n, INF));
  vector count(3, vector<Mint>(n, Mint(0)));
  REP(i, 2) {
    int source = 0;
    mincost[i][source] = 0LL;
    count[i][source] = 1;
    MinHeap<State> que;
    que.push({0, source});
    while (not que.empty()) {
      ...
      que.pop();
      if (cur.cost != mincost[i][cur.node]) continue;
      for (const auto &e : g[cur.node]) {
        i64 new_cost = cur.cost + (i == 0 ? e.cost_s : e.cost_w);
        if (mincost[i][e.to] > new_cost) {
          mincost[i][e.to] = new_cost;
          count[i][e.to] = count[i][cur.node];
          que.push({new_cost, e.to});
        } else if (mincost[i][e.to] == new_cost) {
          count[i][e.to] += count[i][cur.node];
        }
      }
    }
  }
  {
    ...
    while (not que.empty()) {
      State cur = move(que.top());
      que.pop();
      if (cur.cost != mincost[0][cur.node]) continue;
      for (const auto &e : g[cur.node]) {
        i64 summer_cost = mincost[0][cur.node] + e.cost_s;
        i64 winter_cost = mincost[1][cur.node] + e.cost_w;
        if (mincost[0][e.to] == summer_cost and
            mincost[1][e.to] == winter_cost) {
          count[2][e.to] += count[2][cur.node];
          if (not done[e.to]) {
            done[e.to] = true;
            que.push({summer_cost, e.to});
          }
        }
      }
    }
  }
  DEBUG(mincost[0][n - 1], count[0][n - 1]);
  DEBUG(mincost[1][n - 1], count[1][n - 1]);
  DEBUG(count[2][n - 1]);
  return (count[0][n - 1] + count[1][n - 1] - count[2][n - 1]);
}
