...
auto solve() {
  ...
  Compressed<Int> cvals(vals);

  auto f = [&](Int th) -> Int {
    auto dp = vector(H, vector(W, vector(H + W + 2, (Int) kBig)));
    if (g[0][0] >= th) {
      dp[0][0][1] = g[0][0];
    } else {
      dp[0][0][0] = 0;
    ...
    REP(i, H) {
      REP(j, W) {
        ...
        if (i > 0) {
          REP(k, H + W) {
            const auto pval = dp[i - 1][j][k];
            if (pval == kBig) continue;
            if (x >= th) {
              chmin(dp[i][j][k + 1], pval + x);
            } else {
              chmin(dp[i][j][k], pval);
            }
          }
