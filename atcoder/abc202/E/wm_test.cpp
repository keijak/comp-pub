...
struct CompressedWaveletMatrix {
  ...
  explicit CompressedWaveletMatrix(const std::vector<T> &v) : values(v) {
    ...
    for (size_t i = 0; i < v.size(); i++) t[i] = index(v[i]);
 ...
 private:
  inline size_t index(const T &x) const {
...
int main() {
  vector<int> v1 = {0, 3, 2, 3, 0, -50, 3, 2, 2};
  CompressedWaveletMatrix<int> w1(v1);
  DUMP(w1.range_freq(1, 5, 0, 3));
}
