...
auto solve() {
  ...
  U += "_";
  U += T;
  const int ns = ssize(S);
  const int nt = ssize(T);
  const auto Z = ZArray(U);
  DUMP(Z);

  SegmentTree<MinOp> dp(nt + 1);
  dp.set(nt, 0);
  for (int i = nt - 1; i >= 0; --i) {
    const int x = Z[ns + 1 + i];
    const int minval = dp.fold(i + 1, min(i + 1 + x, nt + 1));
    if (minval < kBigVal<int>) {
      dp.set(i, minval + 1);
    }
  }
  out(dp[0] == kBigVal<int> ? -1 : dp[0]);
}
