...
auto solve() {
  ...
  auto dp = vector(1 << n, (Int) kBig);
  dp[0] = 0;
  REP(bits, 1, 1 << n) {
    int pc = popcount(bits) - 1;
    REP(i, n) {
      if (not has_bit(bits, i)) continue;
      unsigned pb = bits & ~(1 << i);
      Int x_cost = abs(A[i] - B[pc]) * X;
      unsigned imask = (1 << i) - 1;
      Int y_cost = popcount(pb & ~imask) * Y;
      chmin(dp[bits], dp[pb] + x_cost + y_cost);
    }
//    BitSet bs(n, bits);
//    DUMP(bs, dp[bits]);
  }
