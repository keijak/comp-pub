...
template<typename T, int kBitWidth = 30>
...
auto solve() {
  int n = in;
  vector<unsigned> a = in(n), b = in(n);
  Compressed<unsigned> ca(a);
  vector<pair<unsigned, unsigned>> ab(n);
  REP(i, n) {
    ab[i] = {a[i], b[i]};
  }
  sort(ALL(ab));
  vector<unsigned> va(n), vb(n);
  REP(i, n) {
    va[i] = ab[i].first;
    vb[i] = ab[i].second;
  }
  WaveletMatrix<unsigned, 30> wm(vb);
  Int ans = 0;
  REP(i, n) {
    unsigned x = va[i];
    unsigned y = vb[i];
    int r = upper_bound(ALL(va), x) - va.begin();
    int val = wm.range_freq(0, r, y, (unsigned) kBig);
    ans += val;
    DUMP(i, x, r, y, val, ans);
  }
  return ans;
}
