...
auto solve() {
  ...
  mcf.add_supply(kSource, H + W);
  mcf.add_demand(kSink, H + W);
  mcf.add_edge(kSource, kSink, 0, H + W, 0);
  auto vx = mcf.add_vertices(H);
  auto vy = mcf.add_vertices(W);
  REP(i, H) {
    mcf.add_edge(kSource, vx[i], 1, W, 0);
  }
  REP(i, W) {
    mcf.add_edge(vy[i], kSink, 1, H, 0);
  }
  REP(i, n) {
    int a = in, b = in, c = in;
    --a, --b;
    mcf.add_edge(vx[a], vy[b], 0, 1, c);
  }
  auto[status, mincost] = mcf.solve();
