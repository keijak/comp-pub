...
auto solve() {
  int n = in, m = in;
  vector<tuple<int, int, int>> tower(n + m);
  REP(i, n + m) {
    int x = in, y = in, c = in;
    tower[i] = {x, y, c};
  }
  ...
  REP(bits, 1 << m) {
    ...
    Compressed<int> cov(vs);
    int V = (int) vs.size();

    vector<tuple<Real, int, int>> edges;
    REP(u, n + m) {
      if (u >= n and not has_bit(bits, (u - n))) continue;
      auto[xu, yu, cu] = tower[u];
      REP(v, u + 1, n + m) {
        if (v >= n and not has_bit(bits, (v - n))) continue;
        auto[xv, yv, cv] = tower[v];
        Real x = abs(xu - xv);
        Real y = abs(yu - yv);
        ...
        edges.emplace_back(d, cov.index(u), cov.index(v));
      }
    }
    sort(ALL(edges));
    UnionFind uf(V);
