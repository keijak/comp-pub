...
auto solve() {
  ...
  int kOffset = 200;
  auto dp = vector(n + 1, vector(400, vector(2, (Int) kBig)));
  dp[0][kOffset][0] = 0;
  REP(i, n) {
    auto[a, b, c] = p[i];
    Int x = mb * a - ma * b;
    ...
    for (int j = -150; j <= 150; ++j) {
      REP(k, 2) {
        if (dp[i][kOffset + j][k] == kBig) continue;
        Int j2 = kOffset + j + x;
        if (0 <= j2 and j2 < 400) {
          chmin(dp[i + 1][j2][1], dp[i][kOffset + j][k] + c);
        }
      }
    }
  }
  Int ans = dp[n][kOffset][1];
  return ans == kBig ? -1LL : ans;
}
