...
i64 solve() {
  ...
  assert(R <= P);
  assert(R > 1);
  const Mint xinv = Mint(X).inv();

  map<int, int> rmin, rmax;
  {
    Mint xr = 1;
    for (int i = 0; i < R; ++i) {
      int v = xr.val();
  ...
  const int q1 = B / R;
  const int r1 = B % R;
  const Mint xinvr = xinv.pow(R);
  const Mint xinv0 = xinvr.pow(q0);

  for (int y = 1;; ++y) {
    Mint z = y * xinv0;
    if (auto it = rmax.find(z.val()); it != rmax.end() and it->second >= r0) {
      return y;
    }
    for (int q = q0 + 1; q < q1; ++q) {
      z *= xinvr;
      if (auto it = rmin.find(z.val()); it != rmin.end()) {
        return y;
      }
    }
    z *= xinvr;
    if (auto it = rmin.find(z.val()); it != rmin.end() and it->second <= r1) {
      return y;
    }
  }
  assert(false);
