...
constexpr int DMAX = 200005;
using DF = DenseFPS<NTTMult<Mint, DMAX>>;

Mint solve() {
  int R = in, G = in, B = in, K = in;
  int X = in, Y = in, Z = in;
  int Rreq = K - Y, Greq = K - Z, Breq = K - X;
  if (Rreq < 0 or Greq < 0 or Breq < 0) return 0;
  if (Rreq + Greq + Breq > K) return 0;
  if (Rreq > R or Greq > G or Breq > B) return 0;
  Factorials<Mint> fs(DMAX);
  DF f = {1};
  vector<Mint> rc(R + 1), gc(G + 1), bc(B + 1);
  for (int i = Rreq; i <= R; ++i) {
    rc[i] = fs.C(R, i);
  }
  f *= DF(rc);
  for (int i = Greq; i <= G; ++i) {
    gc[i] = fs.C(G, i);
  }
  f *= DF(gc);
  for (int i = Breq; i <= B; ++i) {
    bc[i] = fs.C(B, i);
  }
  f *= DF(bc);
  return f[K];
