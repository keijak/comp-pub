...
struct DenseFPS {
  ...
  static constexpr long long dmax() { return Mult::dmax(); }
  ...
  void shift_inplace(int k) {
    if (k > 0) {
      if (size() <= dmax()) {
        coeff_.resize(std::min<long long>(size() + k, dmax() + 1), 0);
      }
...
// T: modint
template<typename T, long long DMAX>
struct NTTMult {
  ...
  using value_type = T;
  static constexpr long long dmax() { return DMAX; }
  ...
  static std::vector<T> invert(const std::vector<T> &x, long long sz = -1) {
    int n = x.size();
...
typename FPS::T bostan_mori(long long k, FPS P, FPS Q) {
  ...
  while (k) {
    ...
    auto S = P * Q2;
    auto T = Q * Q2;
    for (int i = int(k & 1); i < S.size(); i += 2) P.coeff_[i >> 1] = S[i];
...
auto solve() {
  ...
  DF f = {1, 1};
  for (int h = K - 1; h >= 1; --h) {
    f = f * (DF{1} - f.shift(1)).inv(K / h + 1);
    f.coeff_.resize(K / h + 1);
  }
  Mint ans = bostan_mori<DF>(N, f, DF{1} - f.shift(1));
  print(ans);
}
