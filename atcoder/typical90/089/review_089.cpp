...
auto solve() {
  ...
  CompressedWaveletMatrix<uint32_t, 32> wm(a);

  vector<Mint> dp(n + 1), dp_sum(n + 2);
  dp[0] = 1;
  dp_sum[1] = 1;
  int l = 0;
  Int inversion = 0;
  REP(r, 1, n + 1) {
    inversion += wm.range_freq(l, r - 1, a[r - 1] + 1, kBigVal<uint32_t>);
    while (inversion > K) {
      inversion -= wm.range_freq(l + 1, r, 0, a[l]);
      ++l;
    }
    assert(l < r);
    dp[r] += dp_sum[r] - dp_sum[l];
    dp_sum[r + 1] = dp_sum[r] + dp[r];
  }
  print(dp[n]);
}
