...
constexpr int L = 87725;
using BS = std::bitset<L>;

auto solve() {
  int n = in;
  vector<int> W = in(n), B = in(n);

  auto dp = vector(55, vector(55 + 55 * 56 / 2, optional<int>()));
  dp[0][0] = 0;
  dp[0][1] = 0;

  auto grundy = Rec([&](auto self, int w, int b) -> int {
    if (dp[w][b].has_value()) {
      return dp[w][b].value();
    }
    BS s;
