...
auto solve() {
  vector<string_view> S(9);
  REP(i, 9) S[i] = string_view(in);
  auto inside = [&](int x, int y) {
    return 0 <= x and x < 9 and 0 <= y and y < 9;
  };
  auto ok = [&](int r0, int c0, int r1, int c1) {
    if (S[r0][c0] != '#' or S[r1][c1] != '#') return false;
    int x = r1, y = c1;
    int dx = r1 - r0, dy = c1 - c0;
    x += dy;
    y -= dx;
    if (not inside(x, y) or S[x][y] != '#') return false;
    x -= dx;
    y -= dy;
    if (not inside(x, y) or S[x][y] != '#') return false;
    return true;
  ...
  int cnt = 0;
  REP(r0, 9) REP(c0, 9) REP(r1, r0, 9) REP(c1, 9) {
          if (r0 == r1 and c0 >= c1) continue;
          if (ok(r0, c0, r1, c1)) ++cnt;
        }
  out(cnt / 2);
}
