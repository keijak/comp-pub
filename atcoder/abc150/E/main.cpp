...
struct Factorials {
  ...
  Factorials(int n) : fact(n + 1), ifact(n + 1) {
    assert(n > 0 and n < (long long)MOD);
    fact[0] = 1;
...
int main() {
  ...
  Mint ans = 0;
  REP(i, n) {
    Mint now = Mint(4).pow(n - 1 - i);
    now *= 2;  // (S[i],T[i]) = (0, 1) or (1, 0)
    Mint rest = i == 0 ? 1 : (Mint(2).pow(i - 1) * (i + 2));
    // for (int j = 0; j <= i; ++j) {
    //   // how many elements with bigger cost are the same?
    //   rest += Mint(i - j + 1) * fs.C(i, j);
    // }
    now *= rest;
    now *= Mint(2).pow(i);
    ans += now * cost[i];
