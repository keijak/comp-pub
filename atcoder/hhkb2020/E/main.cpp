...
int main() {
  ...
  int nt = 0;
  auto cl = vector(H, vector(W + 1, 0));
  auto cr = vector(H, vector(W + 1, 0));
  auto cu = vector(W, vector(H + 1, 0));
  auto cd = vector(W, vector(H + 1, 0));
  REP(i, H) {
    int ri = H - 1 - i;
    REP(j, W) {
      int rj = W - 1 - j;
      ...
      if (grid[ri][rj] == '.') {
        cr[ri][rj] = cr[ri][rj + 1] + 1;
        cd[rj][ri] = cd[rj][ri + 1] + 1;
      }
    }
  }
  ...
  REP(i, H) {
    REP(j, W) {
      ...
      ans += Mint(2).pow(nt) - Mint(2).pow(nt - m);
    }
    cerr << endl;
  }
  cout << ans << endl;
