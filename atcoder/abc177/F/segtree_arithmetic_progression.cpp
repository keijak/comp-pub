...
struct AssignArithmeticProgressionOp {
  ...
  static T op(const T &x, const T &y) {
    return T{
        std::min(x.min, y.min), std::max(x.max, y.max),
        std::min(x.sum + y.sum, (i64) kBig), x.l, y.r,
    };
  }
  ...
  static T f_apply(const F &f, const T &x) {
    ...
    auto[a, b] = *f;
    if (b == kBig) {
      return T{kBig, kBig, kBig, x.l, x.r};
    }
    i64 width = x.r - x.l;
    i64 lval = a * x.l + b;
    i64 rval = a * x.r + b;
...
int main() {
  ...
  const int H = in, W = in;
  vector<AssignArithmeticProgressionOp::T> init(W);
  REP(i, W) {
    init[i] = {0, 0, 0, i, i};
  }
  LazySegTree<AssignArithmeticProgressionOp> seg(init);
  REP(i, H) {
    int l = in, r = in;
    --l;
    i64 x = kBig;
    ...
    seg.apply(l, r, Line{1, x});
    i64 hor = seg.fold_all().min;
    if (hor == kBig) {
      print(-1);
    } else {
      print(i + 1 + hor);
