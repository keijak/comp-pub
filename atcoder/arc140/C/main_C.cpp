...
int lis(const vector<int> &a) {
  int n = ssize(a);
  SegmentTree<MaxOp> seg(n + 10);
  REP(i, n) {
    int l = seg.fold(0, a[i]) + 1;
    if (seg[a[i]] < l) {
      seg.set(a[i], l);
    }
  }
...
int difflis(const vector<int> &p) {
  const int n = ssize(p);
  vector<int> a(n - 1);
  REP(i, n - 1) {
    a[i] = abs(p[i] - p[i + 1]);
...
auto naive(int X, vector<int> &P) -> vector<int> {
  swap(P[0], P[X - 1]);
  ...
  vector<int> maxp;
  do {
    if (chmax(maxlis, difflis(P))) maxp = P;
  } while (next_permutation(P.begin() + 1, P.end()));
  DUMP(maxlis);
  return maxp;
}

auto greedy(int n) -> vector<int> {
  vector<int> res(n);
  res[0] = 0;
  int d = 1;
  int minval = 0;
...
auto solve() -> vector<int> {
  ...
  REP(flip, 2) {
    ...
    vector<int> ans;
    ans.push_back(X);
    while (not v.empty() or not w.empty()) {
      if (not v.empty()) {
