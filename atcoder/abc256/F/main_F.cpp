...
struct ArithmeticProgressionAddSumOp {
  ...
  static F f_compose(const F &f, const F &g) { return Line{f.a + g.a, f.b + g.b}; }
...
auto solve() {
  ...
  vector<ArithmeticProgressionAddSumOp::T> ini(n);
  REP(i, n) { ini[i] = {0, i, i}; }
  LazySegmentTree<ArithmeticProgressionAddSumOp> seg(ini);
  REP(i, n) {
    seg.apply(i, n, Line{a[i], -a[i] * (i - 1)});
  }
  REP(_, Q) {
    test_case(_, Q);
    int qtype = in;
    if (qtype == 1) {
      ...
      --x;
      Mint d = v - a[x];
      seg.apply(x, n, Line{d, -d * (x - 1)});
      a[x] = v;
    } else {
      int x = in;
      Mint res = seg.fold(0, x).sum;
      print(res);
    }
  }
}
