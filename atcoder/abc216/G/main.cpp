...
auto solve() {
  ...
  sort(ALL(co));
  vector<AssignSumOp::T> ts(n);
  REP(i, n) {
    ts[i] = {0LL, 1};
  }
  LazySegTree<AssignSumOp> seg(ts);
  REP(t, m) {
    int r, l, x;
    tie(r, l, x) = co[t];
    i64 ones = seg.fold(l, r).sum;
    if (ones >= x) continue;
    int p = max_right(seg, l, [&](const AssignSumOp::T ls) {
      return ls.sum + ((r - l) - ls.width) >= x;
    });
    if (p < r) {
      seg.apply(p, r, 1LL);
    }
