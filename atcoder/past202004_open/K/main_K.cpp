...
auto solve() {
  int n = in;
  string S = in;
  vector<Int> C = in(n), D = in(n);
  auto dp = vector(n + 1, vector(n + 5, (Int) kBig));
  dp[0][0] = 0;
  REP(i, n) {
    bool open = S[i] == '(';
    REP(j, i + 1) {
      if (dp[i][j] == kBig) continue;
      if (open) {
        if (j + 1 <= n) chmin(dp[i + 1][j + 1], dp[i][j]);
        if (j - 1 >= 0) chmin(dp[i + 1][j - 1], dp[i][j] + C[i]);
        chmin(dp[i + 1][j], dp[i][j] + D[i]);
      } else {
        if (j - 1 >= 0) chmin(dp[i + 1][j - 1], dp[i][j]);
        if (j + 1 <= n) chmin(dp[i + 1][j + 1], dp[i][j] + C[i]);
        chmin(dp[i + 1][j], dp[i][j] + D[i]);
      }
    }
  }
