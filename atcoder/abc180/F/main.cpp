...
int main() {
  ...
  auto npath = vector(N + 1, Mint(0));
  if (N >= 1) npath[1] = 1;
  if (N >= 2) npath[2] = 1;
  for (int i = 3; i <= N; ++i) npath[i] = npath[i - 1] * i;

  auto ncycle = vector(N + 1, Mint(0));
  if (N >= 2) ncycle[2] = 1;
  for (int i = 3; i <= N; ++i) ncycle[i] = npath[i - 1];

  auto f = [&](int n, int m, int l) -> Mint {
    if (l == 0) return 0;

    auto dp = vector(N + 1, vector(M + 1, Mint(0)));
    dp[0][0] = 1;
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j <= m; ++j) {
        for (int k = 1; k <= l; ++k) {
          ...
          dp[ni][nj] += dp[i][j] * fs.C(n - i - 1, k - 1) * npath[k];
        }
        for (int k = 2; k <= l; ++k) {
          int ni = i + k;
          int nj = j + k;
          if (ni > n or nj > m) break;
          dp[ni][nj] += dp[i][j] * fs.C(n - i - 1, k - 1) * ncycle[k];
        }
      }
    }
    return dp[n][m];
