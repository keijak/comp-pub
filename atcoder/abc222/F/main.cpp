...
struct Task {
  using NV = pair<i64, i64>;
  using EV = i64;

  vector<i64> D;

  explicit Task(vector<i64> d) : D(std::move(d)) {}

  EV id() const { return 0LL; }
  ...
  EV add_edge(const NV &val, const Edge &edge) const {
    return max(val.first, val.second) + edge.co;
  }

  EV merge(const EV &x, const EV &y) const {
    return max(x, y);
  }
};
...
class Rerooter {
 ...
 public:
  explicit Rerooter(Rerootable task, std::vector<std::vector<Edge>> g, int r = 0)
      : task(move(task)),
        n((int) g.size()),
        g(move(g)),
        sub(n),
