...
i64 solve() {
  int R, C, X, Y, D, L;
  cin >> R >> C >> X >> Y >> D >> L;
  vector<vector<Mint>> comb = comb_table(X * Y);

  auto f = [&](int x, int y) -> Mint {
    if (x * y < D + L) return 0;
    return comb[x * y][D + L] * comb[D + L][D];
  };

  Mint inside = f(X, Y);
  if (D + L != X * Y) {
    inside -= 2 * f(X - 1, Y) + 2 * f(X, Y - 1);
    inside += f(X - 2, Y) + f(X, Y - 2) + 4 * f(X - 1, Y - 1);
    inside -= 2 * f(X - 2, Y - 1) + 2 * f(X - 1, Y - 2);
    inside += f(X - 2, Y - 2);
  }
  Mint ans = (R - X + 1) * (C - Y + 1) * inside;
  return ans.val();
}
