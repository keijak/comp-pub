...
auto solve() {
  ...
  auto dp = vector(n + 1, vector(asum + 1, vector(asum + 1, Real(kBig))));
  dp[0][0][0] = 0;
  REP(i, 0, n - 1) {
    REP(j, asum + 1) {
      REP(k, asum + 1) {
        if (dp[i][j][k] == kBig) continue;
        REP(b, asum + 1) {
          if (k + b > asum) continue;
          i64 h = abs(b - j);
          h *= h;
          chmin(dp[i + 1][b][k + b], dp[i][j][k] + sqrt(1 + h));
        }
      }
    }
  }
  return dp[n - 1][0][asum];
}
