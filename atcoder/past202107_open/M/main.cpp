...
auto solve() {
  ...
  g.add_edge(kSource, kSink, n, C);

  REP(i, n) {
    g.add_edge(kSource, 2 * i, 1, 0);
    g.add_edge(2 * i + 1, kSink, 1, 0);
  }
  REP(i, n) {
    REP(j, i + 1, n) {
      i64 cost = abs(a[i] - a[j]);
      g.add_edge(2 * i, 2 * j + 1, 1, cost);
    }
  }

  auto[maxflow, mincost] = g.flow(kSource, kSink, n);
  check(maxflow == n);
  return mincost;
}
