...
auto solve() {
  ...
  REP(i, n) {
    ...
    goods.emplace_back(p % 100, lo);
  }
  DUMP(goods);
  const int m = ssize(goods);
  auto dp = vector(m + 1, vector(100, (i64) -kBig));
  dp[0][0] = 0;
  REP(i, m) {
    auto[r, lo] = goods[i];
    dp[i + 1] = dp[i];
    REP(j, 100) {
      ...
      chmax(dp[i + 1][(j + r) % 100],
            dp[i][j] + lo + ((j + r) / 100) * 20);
    }
  }
  return *max_element(ALL(dp[m]));
}
