...
i64 solve() {
  ...
  vector vars(n, vector<burny::Variable>());
  REP(i, n) {
    string s;
    cin >> s;
    g[i] = s;
    REP(j, n) {
      vars[i].push_back(model.add_variable());
      if (s[j] == 'B') {
        model.add_constraint(~vars[i][j], BIG_COST);
      } else if (s[j] == 'W') {
        model.add_constraint(vars[i][j], BIG_COST);
      }
    }
  ...
  REP(i, n) {
    REP(j, n - 1) {
      model.add_constraint(vars[i][j], ~vars[i][j + 1], -1);
      model.add_constraint(~vars[i][j], vars[i][j + 1], -1);
      model.add_constraint(vars[j][i], ~vars[j + 1][i], -1);
      model.add_constraint(~vars[j][i], vars[j + 1][i], -1);
    }
  }
  auto [result, state] = model.solve();
  cerr << result << ": " << state << endl;
  return -result;
}
