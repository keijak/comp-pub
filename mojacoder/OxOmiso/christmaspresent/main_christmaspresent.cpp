...
class MinCostFlow {
 ...
 public:
  ...
  // cost: gain when objective is MAXIMIZE.
...
int main() {
  ...
  int n, m;
  cin >> n >> m;
  vector<array<pair<int, Int>, 5>> want(n);
  REP(i, n) {
    REP(j, 5) {
      int w;
      cin >> w;
  ...
  MaxGainFlow<int, Int> g;
  const auto v_source = g.add_vertex();
  const auto v_sink = g.add_vertex();
  const auto v_kids = g.add_vertices(n);
  const auto v_toys = g.add_vertices(m);
  g.add_supply(v_source, n);
  g.add_demand(v_sink, n);
  REP(i, n) g.add_edge(v_source, v_kids[i], 1, 1, 0);
  REP(i, n) {
    for (const auto&[w, h]: want[i]) {
      g.add_edge(v_kids[i], v_toys[w], 0, 1, h);
    }
  }
  REP(j, m) {
    int lower, upper;
    cin >> lower >> upper;
    g.add_edge(v_toys[j], v_sink, lower, upper, 0);
  }
  auto[status, gain] = g.solve();
  if (status == Status::INFEASIBLE) {
    cout << -1 << endl;
  } else {
    cout << gain << endl;
