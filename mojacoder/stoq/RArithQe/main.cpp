...
ostream &operator<<(ostream &os, const ArithmeticProgression::T &a) {
  return os << "{min:" << a.min << ", max:" << a.max << ", sum:" << a.sum << ", l:" << a.l << ", r:" << a.r << "}";
}

auto main() -> int {
  ios_base::sync_with_stdio(false), cin.tie(nullptr);
  const int n = in, Q = in;
  vector<ArithmeticProgression::T> init(n);
  REP(i, n) {
    init[i] = ArithmeticProgression::T{0, 0, 0, i, i};
  }
  LazySegTree<ArithmeticProgression> seg(init);
  REP(qi, Q) {
    const int qtype = in;
    int l = in, r = in;
    --l;
    if (qtype == 1) {
      i64 a = in, b = in;
      seg.apply(l, r, ArithmeticProgression::Line{a, b - a * l});
    } else if (qtype == 2) {
