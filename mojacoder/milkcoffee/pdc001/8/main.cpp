...
auto solve() {
  DUMP(sieve.primes.size());
  int n = in;
  vector<int> a(n);
  cin >> a;
  vector<int> ps;
  vector<set<int>> facs(n);
  REP(i, n) {
    int x = a[i];
  ...
  Compressed<int> pc(ps);
  DUMP(pc.values);

  atcoder::mf_graph<int> g(n + pc.size() + 2);
  const int kSource = n + pc.size();
  ...
  auto flow = g.flow(kSource, kSink, pc.size());
  DUMP(flow);
  int ans = flow;
  ans += pc.size() - flow;
  ans += n - flow;
