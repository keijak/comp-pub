...
auto solve() {
  ...
  auto dp = vector(2, vector(K + 1, vector(2, Int(0))));
  dp[0][0][0] = 1;

  REP(i, n) {
    const int curd = N[i] - '0';
    const int i0 = i & 1, i1 = 1 - i0;
    dp[i1].assign(K + 1, vector(2, Int(0)));
    REP(state, K + 1) REP(smaller, 2) {
        auto cur_val = dp[i0][state][smaller];
        ...
        REP(d, 10) {
          ...
          int state2 = state;
          if (d != 0) { state2++; }
          if (state2 > K) continue;

          dp[i1][state2][smaller2] += cur_val;
        }
      }
  }
