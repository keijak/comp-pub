...
struct ArbitraryModMult {
  ...
  static std::vector<T> convolution(const std::vector<T> &x,
    ...
    const int n = std::min<int>(x.size() + y.size() - 1, sz);
    std::vector<T> res(n);
    for (int i = 0; i < n; ++i) {
...
FPS exp(FPS g) {
  int n = g.size();
  std::vector<T> ret(1, 1);
  for (int i = 1; i < n; i <<= 1) {
    std::vector<T> fv(i << 1, 0);
    fv[0] = 1;
    FPS logr = log(FPS(ret), i << 1);
    for (int j = 0; j < (i << 1); ++j) {
      fv[j] += g[j] - logr[j];
    }
    fv.erase(fv.begin(), fv.begin() + i);
    FPS f(fv);
    f *= FPS(ret);
    ret.insert(ret.end(), f.coeff_.begin(), f.coeff_.begin() + i);
  }
  ret.resize(n);
  return FPS(ret);
}
...
FPS pow(FPS f, long long k) {
  ...
  if (c != 1) f /= c;
  f = log(std::move(f));
  f.coeff_.resize(max<int>(n - l * k, 0));
  f = exp(std::move(f) * k);
  if (c != 1) f *= c.pow(k);
